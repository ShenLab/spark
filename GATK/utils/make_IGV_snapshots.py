...
def make_snapshot_filename(region, height, suffix = None):
    ...
    if len(region) >= 4:
        ...
        if suffix == None:
            ...
            return('{}{}.png'.format(name, str(suffix)))
    elif len(region) == 3:
...
def check_for_bai(bam_file):
    '''
    Check to make sure a 'file.bam.bai' or 'file.bai' file is present in the same dir as the 'file.bam' file
    '''
...
def write_batchscript_regions(region_file, IGV_batchscript_file, image_height, suffix, nf4_mode, group_by_strand=False):
    ...
    for region in region_list:
        ...
        append_string("viewaspairs", IGV_batchscript_file)
...
def run_IGV_script(igv_script, igv_jar, memMB):
    ...
    igv_command = "java -Xmx{}m -jar {} -b '{}'".format(memMB, igv_jar, igv_script)
    print('\nIGV command is:\n{}\n'.format(igv_command))
